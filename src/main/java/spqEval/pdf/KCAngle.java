...
public class KCAngle extends PDFPage {
	...
	KCAngle(int posX, int posY, int width, int height){
		super("Local bending", "phaseLUT.txt");
...
//		calcSpaceToTheLeft();
		...
		try {			
			...
			PDFTools.insertTextBoxUpperY(cts, bX0, bY0-pdt.space, "-180", pdt.subDescSize);
			PDFTools.insertTextBoxToRightBound(cts, bX0 + bW, bY0-pdt.space, "180", pdt.subDescSize);
			PDFTools.insertTextBoxXCentered(cts, iX0 + (iW/2), bY0-0.5f*pdt.space-pdt.descSize, "curvature angle (°)", pdt.descSize);
	...
	private void renderKCAngleRepr() {
		String pathOfRawImage = pdt.sourcePath + pdt.expName + "_kcAng.tif";
