...
public class ZFreqFFTPlot extends PDFPlot {
	...
	double xBaseValue = 50, yBaseValuePow, yBaseValueFreq; 
	...
	public ZFreqFFTPlot(int posX, int posY, int width, int height) {
		...
		desc = "local beat frequency in z";
	...
	private void addDescFreq() {
		...
		float numberOfIndicators = (int) (yMaxFreq / yBaseValueFreq);
		...
		PDFTools.insertTextBoxRotatedUpward(cts, x, fY0+fH/2, "z: frequency", pdt.subDescSize);
	...
	private void addDescPow() {
		...
		PDFTools.insertTextBoxRotatedUpward(cts, x, pY0+pH/2, "power of z", pdt.subDescSize);
		PDFTools.insertTextBoxRotatedUpward(cts, x + pdt.space+ pdt.subDescSize, pY0+pH/2, "(µm/Hz)", pdt.subDescSize);
	}
