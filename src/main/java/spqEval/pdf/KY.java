...
import ij.IJ;
import ij.ImagePlus;
import spqEval.lut.PhaseLUT;

public class KY extends PDFPage {
	...
	spqEval.lut.PhaseLUT lut;
	
	KY(int posX, int posY, int width, int height){
		super("Flagellar position", "phaseLUT.txt");
...
//		calcSpaceToTheLeft();
		...
		try {			
			addDesc();
			PDFTools.insertTextBoxUpperY(cts, x, y - pdt.headerSize, "orthogonal to head-midpiece axis", pdt.headerSize);
			...
			PDFTools.insertTextBoxUpperY(cts, bX0, bY0-pdt.space, "-130", pdt.subDescSize);
			PDFTools.insertTextBoxToRightBound(cts, bX0 + bW, bY0-pdt.space, "130", pdt.subDescSize);
			PDFTools.insertTextBoxXCentered(cts, iX0 + (iW/2), bY0-0.5f*pdt.space-pdt.descSize, "y position (µm)", pdt.descSize);
	...
	private void renderKYRepr() {
		String pathOfRawImage = pdt.sourcePath + pdt.expName + "_kY.tif";
		imagePath = pdt.targetPath + name + ".bmp";
		ImagePlus impIn = IJ.openImage(pathOfRawImage);
		ImagePlus impOut = IJ.createImage(name, impIn.getWidth(), impIn.getHeight(), 1, 24);  //RGB with 3*8 bit
		double pixelValue;
		...
		for (int x = 0; x < impIn.getWidth(); x++) {
			for (int y = 0; y < impIn.getHeight(); y++) {
				...
				if (pixelValue == 0) {
					...
					impOut.getProcessor().putPixel(x, y, lut.getRGBValue(pixelValue, 65534));
				}
			}
		}
