...
public class tools{
	...
	public static float getAverageOfRange(float [] values, int startIndex, int endIndex){
		double average = 0.0;	
		for(int x = startIndex; x <= endIndex; x++){
			average += values [x];
		}	
		return (float)(average / (float)(endIndex-startIndex+1));		
	}
	...
	public static float getMinimumWithinRange(float [] values, int firstIndex, int lastIndex){
	...
	public static float getMedianOfRange(float [] values, int firstIndex, int lastIndex){
		double [] medians = new double [lastIndex - firstIndex + 1];
		for(int i = firstIndex; i <= lastIndex; i++){
			medians [i-firstIndex] = values [i];
		}
		...
		if(medians.length%2==0){
			return (float) ((medians[(int)((double)(medians.length)/2.0)-1]
					+medians[(int)((double)(medians.length)/2.0)])/2.0);
		}else{
			return (float) (medians[(int)((double)(medians.length)/2.0)]);
		}		
	}
	...
	public static float getMaximumWithinRange(float [] values, int firstIndex, int lastIndex){
